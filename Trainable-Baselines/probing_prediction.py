...
from helper_probing import cluster
...
def predict_causal_counterpart(parallel_model, dev_ab, dev_ba, device, batch_size):
...
def predict_with_inner_monologue_ldc(parallel_model, dev_ab,  device, batch_size):
    n = dev_ab['input_ids'].shape[0]
    indices = list(range(n))
    # new_batch_size = batching(n, batch_size, len(device_ids))
    # batch_size = new_batch_size
    ...
    with torch.no_grad():
        for i in tqdm(range(0, n, batch_size), desc='Predicting'):
            batch_indices = indices[i: i + batch_size]
            scores_ab = forward_ab_old(parallel_model, dev_ab, device, batch_indices, ann_attn_hidden_logits = False)
            #scores_ba = forward_ab(parallel_model, dev_ba, device, batch_indices)
            all_scores_ab.append(scores_ab.detach().cpu())
            #all_scores_ba.append(scores_ba.detach().cpu())
...
def predict_with_inner_monologue_ldc(parallel_model, dev_ab,  device, batch_size):
    ...
    with torch.no_grad():
        for i in tqdm(range(0, n, batch_size), desc='Predicting'):
            batch_indices = indices[i: i + batch_size]
            scores_ab = forward_ab_old(parallel_model, dev_ab, device, batch_indices)
            #scores_ba = forward_ab(parallel_model, dev_ba, device, batch_indices)
            all_scores_ab.append(scores_ab.detach().cpu())
            #all_scores_ba.append(scores_ba.detach().cpu())
...
def predict_trained_model(mention_map, model_name, linear_weights_path, test_pairs, text_key='bert_doc', max_sentence_len=1024, long=False):
    ...
    scores_ab, scores_ba = predict_causal_counterpart(parallel_model, test_ab, test_ba, device, batch_size=64)
...
def save_dpos_scores(dataset, split, dpos_folder, heu='lh', threshold=0.999, text_key='bert_doc', max_sentence_len=1024, long=True):
    ...
    scores_ab, scores_ba, pairs = predict_trained_model(evt_mention_map, bert_path, linear_weights_path, test_pairs, text_key, max_sentence_len, long=False)
